...
export async function loadPostList(opts: {
  ...
  if (APP_RUNTIME !== "blueboat") return mockPostList().map(x => {
    x.body = "";
    return x;
  });
...
FROM post
${opts.showPrivate ? "" : "WHERE is_public = 1"}
...
export async function loadPost(opts: {
  ...
  if (APP_RUNTIME !== "blueboat") return mockPostList().find(x => opts.id ? x.id === opts.id : x.shortKey === opts.shortKey) || null;

  const db = opts.ctx.getDatabase();
  const rows = await db.exec(`
SELECT id, short_key, author, title, body, cast(is_public as signed integer), unix_timestamp(inserted_at)
FROM post
...
${opts.showPrivate ? "" : " and is_public = 1"}
  ...
  return rows.map(([id, short_key, author, title, body, isPublic, createdAt]) => ({
    id: id!,
    shortKey: short_key!,
    author: author!,
    title: title!,
